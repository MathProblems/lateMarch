...
if __name__ == "__main__":
    ...
    if len(sys.argv)<2:
        print("args");exit()
    if sys.argv[1]=='asmd':
        f2 = open("data/3.24.verbs.asmd",'wb')
        ...
        f2 = open("data/3.24.verbs.as",'wb')
        wps = open('add.problems').readlines()
        addlen = len(wps)
        wps.extend([x for x in open('sub.problems').readlines()])
    elif sys.argv[1]=='md':
        f2 = open("data/3.24.verbs.md",'wb')
    ...
    for j in range(len(wps)):
        ...
        verb = allnumbs[prblmnumbs[1]].verb
        print(verb)
        if j < addlen: verbs[0].append(verb)
        else: verbs[1].append(verb)
